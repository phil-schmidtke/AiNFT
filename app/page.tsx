...
export default function Home() {
  ...
  async function createImage() {
    ...
    if (image_url && image_url?.length > 2) {
      setLoading(false)
      setImage(image_url)
      setShowImage(true)
    }
  ...
  return (
    <>
          ...
          <div className="fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-50 overflow-hidden bg-gray-700 opacity-75 flex flex-col items-center justify-center">
            <div className="w-96 h-96 relative z-0 text-center">
              ...
              <div className="flex mt-4">
                <input value={input} onChange={(e) => setInput(e.target.value)} className="bg-transparent border-2 border-white rounded-xl p-4 mx-auto w-full" placeholder="5 words here.." />
              </div>
              <div className="flex mt-4">
                <button onClick={() => createImage()} className="mx-auto">Mint NFT</button>
              </div>
            </div>
          </div>
        ) : (<></>)
      ...
      {
        showImage ? (<div className="fixed top-0 left-0 right-0 bottom-0 w-full h-screen z-50 overflow-hidden bg-black flex flex-col items-center justify-center">
          <img src={image} />
        </div>) : <></>
      }
